...
vEnv = Envelope.new(0.001, 0.2)
fEnv = Envelope.new(0.001, 0.025)

filter = MoogFilter.new


verb = GVerb.new(SFREQ, max_room_size: 200.0, room_size: 30.0, rev_time: 0.8, damping:0.3, spread: 15.0, input_bandwidth: 0.5, early_level:0.8, tail_level: 0.5)
...
File.open('debug.txt', 'w') do |debug|
  (SFREQ*5).times do |sample|
    ...
    v = filter.run(v, 0.05 + (fEnv.run(t) * 0.2), 0.5)
    v *= 0.8 * vEnv.run(t)
